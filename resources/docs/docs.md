...
The `home-routes` handler in the `diopart.routes.home` namespace
...
The `home-page` function will in turn call the `diopart.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `diopart.handler` namespace:
...
Request middleware functions are located under the `diopart.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `diopart.dev-middleware` namespace found in
